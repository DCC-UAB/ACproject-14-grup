...
def grid_search_xgb(X_train, y_train):
    ...
    """
    param_grid = {
        'n_neighbors': [1, 2, 3,4, 5],  # Número de veïns
        'weights': ['uniform'],                 # Pesos dels veïns
        'metric': ['manhattan'],  # Mètriques de distància
        'p': [1],                                  # Paràmetre per a la mètrica Minkowski
        'algorithm': ['auto', 'ball_tree', 'kd_tree', 'brute'], # Algorismes utilitzats
        'leaf_size': [25, 30, 35]                   # Mida de les fulles per als arbres
    }

   
    knn = KNeighborsClassifier()
    grid_search = GridSearchCV(estimator=knn, param_grid=param_grid, cv=5, scoring='f1_weighted', verbose=2, n_jobs=-1)
...
if __name__ == "__main__":
    ...
    cami_csv_3s = current_dir.parent / "csv classification" / "features_3_sec_top20_KNN.csv"
    ...
    for data, tipus in [(data3s, "3 seconds")]:  # Diferents datasets
        ...
        # millor_model = KNeighborsClassifier(n_neighbors=19)
        # millor_model = grid_search_xgb(X_train, y_train)
        # millor_model = random_seach_hyperparameters(X_train, y_train)

        millor_model = KNeighborsClassifier(n_neighbors=1, algorithm='ball_tree', leaf_size=25, metric='manhattan', p=1, weights='uniform')

        # Avaluar i guardar resultats del model optimitzat
        model_assess_to_json(millor_model, X_train, X_test, y_train, y_test, "Optimized KNN", resultats, dataset=tipus)

    # Guarda els resultats al fitxer JSON
    guardar_resultats_a_json(resultats, "resultats_KNN_best_hyperparametersP3.json")
