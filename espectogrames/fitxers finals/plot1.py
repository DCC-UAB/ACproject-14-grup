...
def generar_comparacio_plots(resultats_original, resultats_augment, resultats_hog):
    ...
    accuracies_original = [resultats_original[model].get("roc_auc", 0) for model in models]
    accuracies_augment = [resultats_augment[model].get("roc_auc", 0) for model in models]
    accuracies_hog = [resultats_hog[model].get("roc_auc", 0) for model in models]
    ...
    plt.ylabel("ROC-AUC")
    plt.title("Comparació de ROC-AUC: Original vs Augmentat vs HOG Features")
    plt.legend()
    plt.tight_layout()

    # Guardar el plot com a fitxer d'imatge
    plt.savefig("comparacio_roc_auc.png", dpi=300)  # Guarda com a imatge PNG
